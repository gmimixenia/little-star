...
screen main_menu():
    ...
    textbutton _("Начнем?") xalign  0.08  yalign  0.38 action Start()

    textbutton _("Загрузить") xalign  0.22  yalign  0.73 action ShowMenu("load")

    textbutton _("Настройки") xalign  0.46  yalign  0.33 action ShowMenu("preferences")

    textbutton _("Об игре") xalign  0.63  yalign  0.62 action ShowMenu("about")

    if renpy.variant("pc"):

        ## Помощь не необходима и не относится к мобильным устройствам.
        textbutton _("Тебе помочь?") xalign  0.85  yalign  0.22 action ShowMenu("help")

        ## Кнопка выхода блокирована в iOS и не нужна на Android.
        textbutton _("Выход") xalign  0.93  yalign  0.84 action Quit(confirm=not main_menu)
...
screen about():
    ...
    use game_menu(_("Об игре"), scroll="viewport"):
        ...
        vbox:
            ...
            label "[config.name!t]"
            text _("Версия [config.version!t]\n *** \n Все мы надеемся, что однажды в нашей жизни произойдет ЧУДО. А что если на самом деле это Чудо не так радужно, как мы думаем?")
...
define gui.about = "Все мы надеемся, что однажды в нашей жизни произойдет ЧУДО. А что если на самом деле это Чудо не так радужно, как мы думаем?"
